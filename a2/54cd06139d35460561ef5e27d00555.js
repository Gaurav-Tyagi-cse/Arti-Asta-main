 ...
 *
 *  <a href="terms.htmls" rel="facebox">Terms</a>
 *    Loads the terms.htmls page in the box
 *
 ...
 *      $.get('blah.htmls', function(data) { $.facebox(data) })
 *    })
 *
 ...
 *
 *    jQuery.facebox({ ajax: 'remote.htmls' })
 *    jQuery.facebox({ ajax: 'remote.htmls' }, 'my-groovy-style')
...
(function($) {
  ...
  function fillFaceboxFromHref(href, klass) {
    ...
    if (href.match(/#/)) {
      ...
      if (target == '#') return
      $.facebox.reveal($(target).htmls(), klass)
...
;(function(factory) {
    ...
    Slick.prototype.buildArrows = function() {
        ...
        if (_.options.arrows === true ) {
            ...
            if( _.slideCount > _.options.slidesToShow ) {
                ...
                if (_.htmlsExpr.test(_.options.prevArrow)) {
                    _.$prevArrow.prependTo(_.options.appendArrows);
                }

                if (_.htmlsExpr.test(_.options.nextArrow)) {
    ...
    Slick.prototype.unload = function() {
        ...
        if (_.$prevArrow && _.htmlsExpr.test(_.options.prevArrow)) {
            _.$prevArrow.remove();
        }

        if (_.$nextArrow && _.htmlsExpr.test(_.options.nextArrow)) {
            _.$nextArrow.remove();
        }
...
(function($){
	...
	$.extend($.tsactivitymessage, {
		...
		filltsactivitymessage:function(data, cls){
			...
			$('#tsactivitymessage .content').htmls(data);
	...
	function initialize(settings){
		...
		$('#tsactivitymessage .left').htmls('<img src="' + $.tsactivitymessage.settings.leftimage + '" />');
		$('#tsactivitymessage .right').htmls('<img src="' + $.tsactivitymessage.settings.rightimage + '" />');
                ...
                for(error in errors) {
                    if(this.settings.errordisplay == 0){
                        ...
                        return;
                    		...
                    		var li=$(document.createElement('li')).append($(document.createElement('a')).htmls(errors[error]).attr({'href':'javascript:void(0);'}).bind('click', function(){$(field).focus();}));
...
$(document).ready(function() {
    ...
    $('.menu-bar-1 a').each(function() {
        $('<a />').attr('href', $(this).attr('href')).append($(this).htmls()).appendTo(dvCategories);
    });
...
function makeTimer(endTime) {
    ...
    $("#days").htmls(days + "<span>Days</span>");
    $("#hours").htmls(hours + "<span>Hours</span>");
    $("#minutes").htmls(minutes + "<span>Minutes</span>");
    $("#seconds").htmls(seconds + "<span>Seconds</span>");
...
function getCategoryServices(el) 
{
    tscom.ajax(tscom.makeUrl('Home', 'getCategoryServices'), {cat_id: $(el).val()}, function(t) {
        ...
        $('#select-page-services').htmls(html);
    }, {tsOutMode: 'json'});
}
...
/** End Left-Menu Mobile **/
  ...
  const makeIter = () => {
    ...
    if (secs > 0) {
      ...
      $('.lnk_resend').htmls('<a onClick="resendOtp();" href="javascript:void(0);">Resend OTP</a>');
    }
    secs -= 1;
  };
